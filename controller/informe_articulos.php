...
class informe_articulos extends fbase_controller
{
    ...
    public function generar_pdf_list(){
        ...
        if (count($famiaux['list'])>0) {
            ...
            foreach ($famiaux['list'] as $i => $fa) {
                ...
                $pdf->SetXY(10,23);           
                $pdf->cell(70,9,$descripcion,0,0,"L");
                ...
                $pdf->SetFont('Oganesson','',25);
                $pdf->SetXY(2,1);           
                $pdf->MultiCell(120,8,$famiaux['principal'][$i],0,"L");
                ...
                foreach ($articulo as $o => $art) {
                    ...
                    if (!$art->bloqueado&&$art->stockfis) {
                        ...
                        if (file_exists($url)) {
                            ...
                            $pdf->Image(__DIR__.'/../view/image/4-04.png',$xaux2,$yaux,50,55);
                        }
                        
                        $pdf->SetXY($xaux,$yaux+55);
                        $pdf->SetFont('Oganesson','',9);
                        $pdf->SetTextColor(0,0,0);

                        $this->verificar_titulo($pdf,$yaux,$art->descripcion,30);

                        $pdf->SetXY($xaux,$yaux+64);
                        $pdf->SetFont('Oganesson','',15);
                        $pdf->SetTextColor(255,255,255);
                        $pdf->cell(50,7,$this->show_precio($art->pvp_iva()),0,0,"C",true);
    ...
    public function verificar_titulo(&$pdf,$y,$titulo,$hasta){
        ...
        if (strlen($titulo) > $hasta) {
            ...
            $pdf->MultiCell(50,5,$resstr,0,"L");
        }else{
            $resstr .= $titulo;
            $resstr = strtoupper($resstr);
            $pdf->Cell(50,10,$resstr,0,0,"L");
        }
    }
